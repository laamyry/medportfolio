      ...
      <div className="relative flex w-full">
        ...
        <div className="flex-1 flex items-center justify-center px-7 absolute top-0 left-0 bottom-0 z-10 3xl:ml-36">
          <div className="sm:flex sm:flex-col sm:items-center sm:justify-center p-8 rounded-lg max-w-lg transition-all duration-300">
            ...
            <p className="uppercase w-fit pr-6 text-xl mt-3 sm:text-center bg-primary sm:rounded-full transition-all duration-300 text-gray-800 p-3 rounded-tr-full rounded-br-full">I’m a Graphic & Motion Designer</p>
            <button className="bg-primary px-6 text-gray-800 w-fit text-xl py-3 rounded-full mt-4 hover:bg-gray-900 hover:text-white transition ease-in-out duration-500">
              <Link href="#contact" className="hover:text-primary transition ease-in-out duration-500">
                Hire Me
              </Link>
            </button>
          </div>
        ...
        <div className="flex-1 relative">
          <Image
            src="/images/wallpaperflare.webp"
            alt="Profile"
            layout="fill"
            objectFit="cover"
            quality={75} // Adjust the quality setting if needed
            className="absolute top-0 right-0"
            priority={true} // Optional: This forces the image to load faster if it is above the fold
          />
        </div>
      </div>
    </section>
